    ...
    return (
        <div className="max-w-2xl w-full mx-auto space-y-4">
            <form onSubmit={handleSubmit} className="relative group space-y-3">
                <div className="relative">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={cn(
                                "block w-full p-4 md:p-6 pr-16 md:pr-36 text-base md:text-lg rounded-lg border-2 border-transparent bg-gray-900 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all",
                                "disabled:opacity-50 disabled:cursor-not-allowed",
                                "shadow-[0_0_20px_rgba(168,85,247,0.1)] focus:shadow-[0_0_30px_rgba(168,85,247,0.2)]"
                            )}
                        />
                        {/* Mobile: Only sparkle button inline | Desktop: Both buttons inline */}
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-2">
                            <button
                                type="button"
                                onClick={handleSurpriseMe}
                                disabled={isLoading}
                                className="h-10 w-10 md:h-12 md:w-12 flex items-center justify-center bg-gray-800 text-purple-400 rounded-md hover:bg-gray-700 transition-colors border border-purple-500/30"
                                title="Surprise Me"
                            >
                                <Sparkles className="w-5 h-5" />
                            </button>
                            {/* Desktop-only: Find Vibe button inline */}
                            <button
                                type="submit"
                                disabled={!input.trim() || isLoading}
                                className="hidden md:flex h-12 px-6 bg-white text-gray-900 rounded-md font-medium hover:bg-gray-100 transition-colors items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                            >
                                {isLoading ? (
                ...
                {/* Mobile-only: Full-width Find Vibe button below */}
                <button
                    type="submit"
                    disabled={!input.trim() || isLoading}
                    className="md:hidden w-full h-12 bg-white text-gray-900 rounded-lg font-medium hover:bg-gray-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                >
                    {isLoading ? (
                        <div className="w-5 h-5 border-2 border-gray-900 border-t-transparent animate-spin rounded-full" />
                    ) : (
